...
**6 · 🚪 Tests first (RED)** — write logic + UI-behavior tests before impl. **Coverage unit = grid cell, not case:** every equivalence class, boundary, and branch-complement from `00-gap-analysis.md` gets **its own test** (a case that bundles classes → one test each; collapsing them thins coverage silently). Each must fail for the **right reason** — and **prove it**: a test that passes against unimplemented/stub code is trivial; confirm it goes RED *because the asserted behavior is absent*, not for a wiring error (a test that can't be written → back to Phase 3). Record in `04-test-doc.md` (cite each test's cell). Hand over design+tests → on approval set `testsApproved:true` (unblocks impl).
...
**11 · Comprehensive verify** — full suite + audit: **cell coverage** (every `00-gap-analysis.md` cell → case → test, zero uncovered), conformance, traceability filled, logic/UI separation. The `spec-verifier` agent confirms each test **actually asserts its cell** (would fail if the behavior regressed — defaults to refuting, to catch trivial always-green tests); `scripts/verify-workflow.js` for fan-out. Write `07-verify.md`.
