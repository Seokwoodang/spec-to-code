...
| **시작 동작** | 기획서를 0부터 정규화 | 새 기획서 ↔ 이전 확정명세(A) **diff** + 이전 산출물 로드 + **막힌 항목(F) 재점검** |
...
> 💡 업데이트가 제대로 돌려면 첫 개발이 이 플로우로 진행돼 `docs/spec-to-code/<기능명>/` 에 산출물(특히 추적 매트릭스 B)이 남아 있어야 합니다. 이 플로우 없이 만든 기존 코드를 업데이트하려면, 먼저 기존 코드+테스트에서 A/B를 역으로 만들어두고 진행합니다.
