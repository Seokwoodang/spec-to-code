...
- **A completed/static HTML site as the "spec"** (markup + styles, zero behavior) — common when design hands off a built page. Treat it as **two things at once**: (1) the *visual spec* — it answers layout/appearance gaps a text spec couldn't, and becomes the screenshot baseline; (2) a **reusable asset** — its markup/CSS is adapted into the implementation (split into components, keep class names/styles), **not re-created from scratch**. Everything dynamic — interactions, state, validation, loading/empty/error, data fetching, routing — is **unstated → gaps** for the user to resolve at Gate 1. The design (Phase 5) records how the HTML decomposes into components and where behavior/state/API wire in.
