...
**11 · Comprehensive verify** — full suite + audit: **cell coverage** (every `00-gap-analysis.md` cell → case → test, zero uncovered), conformance, traceability filled, logic/IO separation. The `spec-verifier` agent confirms each test **actually asserts its cell** (would fail if the behavior regressed — defaults to refuting, to catch trivial always-green tests); `scripts/verify-workflow.js` for fan-out. Write `07-verify.md`.
