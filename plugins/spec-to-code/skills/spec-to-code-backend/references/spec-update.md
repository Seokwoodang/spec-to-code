...
- A version folder with `v*/01-working-spec.md` (+ `v*/02-resolved-spec.md`) exists for this feature → **UPDATE mode**. Create the next `v(N+1)/`; diff against the **latest `vN/01-working-spec.md`**.
...
In update mode, two saved files do the heavy lifting: the **`01-working-spec.md` snapshot** is the baseline to diff against (the user's original file may have moved or been a transient paste), and the prior **Traceability Matrix (B)** maps every spec case to its tests, code, and screenshots, so impact analysis is a reverse lookup rather than a re-read of the whole codebase.

## The delta path

### U1 — Ingest new spec + load priors
Normalize the new spec (any format — `references/spec-ingestion.md`) and **archive its original verbatim** into `source/<date>-original.<ext>` alongside the prior run's original. Load prior A/B/C/D/E/F and the saved **`01-working-spec.md`** snapshot (the baseline to diff against; if the prior and new originals share a format, an original-vs-original glance is a useful cross-check, but the authoritative diff is on the normalized working spec). **Surface the deferred list to the user** (the proactive resume check from Phase 1): present every open blocked/deferred item and ask which to take on now — flag those whose revisit trigger has plausibly fired (backend shipped, milestone arrived). Promote the chosen items into this update's scope; leave the rest parked with their triggers intact. This is an active prompt, not a silent re-check — a "not now" from a prior run is always put back to the user.

### U2 — Spec diff
Compare the latest `vN/01-working-spec.md` (old) vs the newly normalized spec (new) and classify each requirement change. Ignore pure wording/format churn. Save the new normalized spec as `v(N+1)/01-working-spec.md` (the old version folder is left intact — full history per version); the next update diffs against this latest one.
...
### U3 — Impact analysis (via the traceability matrix)
For each change, reverse-look-up the traceability matrix to find the affected **spec-cases → tests → code units → screenshots**. The union is the **impact set** — what must change, plus what must be re-verified for regression.
...
Added/modified requirements may introduce new gaps — run gap analysis on the delta only and resolve with the user. Produce **`v(N+1)/00-behavior-grid.md`**: carry the prior grid forward and add/modify the cells the delta touches (new axes, new combinations, new complements), every cell decided. The hook gates `v(N+1)/02-resolved-spec.md` on this file existing, so the delta grid is mandatory too — diff it, don't skip it. Open a new **`CHANGELOG.md` entry** for this run capturing the spec diff (U2), the impact set (U3), and a **Tasks checklist** (the concrete to-do for this update). Then **Gate 1 (hard stop) on the delta**: present the changed decisions (updated A) + changed test plan (updated D) **+ the CHANGELOG Tasks checklist** for approval before touching code — this checklist is the user-facing "here's what this update will do."

**Deferring a delta change ("changed in the spec, but not now"):** at this gate the user may decide a changed/added requirement should not be built yet. That decision is not a silent skip — it goes to **the deferred list** with a concrete revisit trigger (same no-silent-drop rule as the main flow), its planned test is marked skipped/pending pointing to the F id, and its traceability row is marked `deferred`. **Critical:** the `01-working-spec.md` snapshot is still overwritten with the *new* spec (it records what the spec now says), so the deferral would be invisible to the next diff — F + the `deferred` traceability-row are the only record that the code lags the spec here. They are re-checked at U1 of every future update.
...
- **Comprehensive verify**: conformance is checked against the **new full A** (not just the delta) so nothing fell through; confirm the traceability matrix has no orphan/`TODO` rows; regression intact.
