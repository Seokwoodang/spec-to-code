...
**12 · 🚪 Gate 2** — compile `08-completion.md` + verify + traceability + reviews + `deferred.md`; report; surface open deferred items. On approval `active:false`. Never commit unless told.
...
Bundled PreToolUse hook (`hooks/gate-guard.mjs`), staged: `designApproved:false` → all code & test edits blocked (only doc home + state file writable); `designApproved:true, testsApproved:false` → tests allowed, **impl blocked**; `testsApproved:true` → all allowed. Scoped & fail-open: no active state file → complete no-op; any error allows the edit. Makes "no server code without an approved design" impossible to skip.
