...
## 2. 갭 분석 — 4줄이 15개 구멍으로 (`00-behavior-grid.md`, 필수)
...
## 10 — 🔁 리뷰 루프 (독립 리뷰어)
각 라운드 = **두 파트**: (a) **blind 발견** — 별도 컨텍스트의 독립 리뷰어가 *현재 diff + resolved-spec만* 보고(이전 라운드는 안 봄) 코드를 새로 읽어 PR-급 리뷰 작성, (b) **closure 확인** — 직전 라운드의 fix 처분 지적이 실제로 반영됐는지 메인 루프가 대조(이건 blind 아님, 명시). (`06-review/r1.md`, `r2.md`):
- 라운드1(blind) → 사용자가 R1(언마운트 cleanup)·R3(aria) **fix** 처분 → 수정
- 라운드2 → **closure**: R1·R3 반영 ✅ 확인(non-blind 표기) + **blind 발견**: 이전 라운드를 못 본 새 리뷰어가 **새 버그 4개 발견** (id 미인코딩, Enter preventDefault 누락, 비배열 응답, console.error 미단언 등)

> 📌 교훈: 작성자가 자기 코드 리뷰 ❌. 독립 리뷰어는 **이전 라운드도 안 보고** 새로 읽어야 anchoring 없이 진짜 버그가 잡힘. 이전 지적 반영 확인은 별도 non-blind 단계로 분리. 라운드마다 **새 리뷰 파일**.
