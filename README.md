...
├── v1/                기획서 v1 산출물 세트
│   ├── 01-working-spec.md   정규화 스냅샷 (다음 버전 diff 기준)
│   ├── 02-resolved-spec.md  확정 명세 (Gate 1)
│   ├── 03-design.md         완벽한 개발문서 — 파일·함수·동작 전부 (Phase 5)
│   ├── 04-test-doc.md       테스트 계획 → 리포트
│   ├── 05-traceability.md   추적 매트릭스 (명세↔테스트↔코드)
│   ├── 06-review/r1.md…     독립 리뷰 라운드 (전부 보존)
│   ├── 07-verify.md         종합 검증 (Phase 11)
│   └── 08-completion.md     완료 보고 (Gate 2)
└── v2/                업데이트마다 새 버전 폴더
```
> 업데이트는 새 `v(N+1)/` 를 만들고 직전 `vN/01-working-spec.md` 와 diff. 코드·테스트는 doc home이 아니라 프로젝트 자체 위치에.
...
- **스크린샷·baseline**: Playwright가 두는 곳(e2e 테스트 옆 `*-snapshots/`)에 저장·커밋되고, 08-completion.md 가 링크로 참조.

| 체크포인트 | 받는 것 | 하는 것 |
|---|---|---|
| 🚪 **Gate 1** (코딩 전) | 갭 질문 + 확정 명세(02-resolved-spec.md) + 테스트 계획(D) | 빈 곳 답하고 → 진행 승인 |
| 🔁 **리뷰 루프** (반복) | 리뷰 문서(06-review/) | finding별 accept/reject/defer → 통과 시 다음 |
| 🚪 **Gate 2** (완료 후) | 완료 문서(08-completion.md) + 테스트 결과 + 스크린샷 + 미결(deferred.md) | baseline 승인 → 최종 OK |
...
| **시작 동작** | 기획서를 0부터 정규화 | 새 기획서 ↔ 저장된 **`01-working-spec.md` 스냅샷** **diff** + 이전 산출물 로드 + **막힌 항목(deferred.md) 재점검** |
| **갭/Gate 1** | 기능 전체의 빈 곳을 다 질문 | **바뀐 부분의 갭만** 질문 + 변경이 건드린 기존 일탈/가정 재확인 |
| **작업 범위** | 전체 구현 | **델타만** — 영향 분석(05-traceability.md 역색인)으로 바뀐 케이스·테스트·코드만 손댐 |
...
- **저장 위치**: 첫 개발 Phase 1에서 기획서를 정규화한 **스냅샷을 `docs/spec-to-code/<기능명>/01-working-spec.md` 에 저장**합니다. 원본 기획서는 사용자가 두던 곳(예: `doc/` 트리, Figma, 붙여넣기)에 그대로 두고 **경로/링크만 기록** — 원본은 옮기거나 수정하지 않습니다. 업데이트 때 diff 의 기준은 이 스냅샷입니다 (원본은 이동·수정·휘발될 수 있으므로).
...
> 💡 업데이트가 제대로 돌려면 첫 개발이 이 플로우로 진행돼 `docs/spec-to-code/<기능명>/` 에 `01-working-spec.md` 와 산출물(특히 추적 매트릭스 05-traceability.md)이 남아 있어야 합니다. 이 플로우 없이 만든 기존 코드를 업데이트하려면, 먼저 기존 코드+테스트에서 working-spec·resolved-spec·traceability를 역으로 만들어두고 진행합니다.
...
| 2 | 갭 분석 | 갭 목록 (전수) | — |
| 3 | 갭 해소 | `02-resolved-spec.md` (확정 명세) | — |
| 4 | **🚪 Gate 1** | 사용자가 `02-resolved-spec.md` 승인 | **하드 스톱** |
| 5 | 설계 | `03-design.md`(완벽한 개발문서) + `05-traceability.md`(초안) | 🟠 |
| 6 | **🚪 테스트 먼저** | 테스트(RED) + `04-test-doc.md` → 구현 전 승인 | **하드 스톱** |
| 7 | 로직 구현 | 로직 테스트 **GREEN** | — |
| 8 | UI 구현 | UI 동작 테스트 **GREEN** | — |
| 9 | 시각 검증 | Playwright 스크린샷 → baseline 후보 | 🟠 |
| 10 | **🔁 리뷰 루프** | **독립 리뷰어** → `06-review/r1.md, r2.md…`; 사용자 처분 → 재리뷰 | **하드 스톱** |
| 11 | 종합 검증 | `05-traceability.md` 채움 + `07-verify.md`; spec정합·전체suite·분리 | — |
| 12 | **🚪 Gate 2** | `08-completion.md` + `07-verify.md` + `05-traceability.md` + `06-review/` + `deferred.md` 보고 | **하드 스톱** |
...
- **`02-resolved-spec.md`** — 모든 결정·케이스·엣지·에러를 못박은 계약 (Gate 1)
- **`03-design.md`** — 완벽한 개발문서: 파일 구조·함수 목록+시그니처·버튼 동작·상태·에러분기 전부 (Gate 전 승인)
- **`04-test-doc.md`** — 테스트 계획(케이스 목록) → 리포트(결과·커버리지)
- **`05-traceability.md`** — 명세 ↔ 테스트 ↔ 코드 ↔ 통과여부 (커버리지 증명; 빈 칸 = 미완료)
- **`06-review/r1.md, r2.md…`** — **독립 리뷰어** 의 라운드별 리뷰 (전부 보존)
- **`07-verify.md`** — 종합 검증 리포트
- **`08-completion.md`** — 요약·결정·실행/검증법·스크린샷·잔여
...
- 실행 중인 작업(`.spec-to-code-state.json` 의 `active:true`)이 단계별로 **03-design.md 미승인 → 코드·테스트 전부 차단, 테스트 미승인 → 구현 차단** (doc home 문서·상태파일은 편집 허용)
