...
This is **not optional and not gated on spec size.** Every run produces `00-gap-analysis.md` (the gap matrix) before the resolved spec. "The spec looks well-defined" is **not** grounds to skip it — re-architecture (sync→async, in-memory→server, single-page→routed) introduces branches the source never had, and those are exactly what eyeballing misses. Prose "be exhaustive" has failed before; the forcing function is the **filled table**, where "did we think of every case?" becomes "is any cell empty?".
